...
use Modules\Qrcode\Http\Requests\UpdateLinkRequest;
use Modules\Qrcode\Models\Link;
use Modules\Qrcode\Models\LinkPixel;
use Modules\Qrcode\Models\Pixel;
use Modules\Qrcode\Models\Space;
...
class LinkController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $links = Link::with('domain', 'space')
            ...
            // pagination making    
            $pagination = Link::paginate(10)->onEachSide(2);



        return view('Qrcode::links.container', ['view' => 'list', 'links' => $links, 'spaces' => $spaces, 'domains' => $domains, 'pixels' => $pixels,'pagination' => $pagination]);
    }
    ...
    public function destroy(Request $request)
    {
        ...
        if(!empty($request->link_id)){
            $links = Link::find($request->link_id)->delete();
